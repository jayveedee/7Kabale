...
class TestGameLogic(TestCase):
    ...
    def test_updateM_logic(self):
        ...
        self.assertListEqual(self.gl.update_logic_move(["5", "h", "2", "0", "T", "T", "NO"]),
                             ["4 s", "5 h", "6 c", "7 d", "8 s"])
        self.assertListEqual(self.gl.update_logic_move(self.gl.calculate_move()), ["1 s"])
        self.assertListEqual(self.gl.update_logic_move(self.gl.calculate_move()), ["13 s"])
        self.gl.logicTableauCardPiles.get(5).clear()
        self.gl.logicTableauCardPiles.get(5).append("4 h")
        self.assertListEqual(self.gl.update_logic_move(self.gl.calculate_move()), ["3 h", "2 h", "1 h"])
        self.assertListEqual(self.gl.update_logic_move(self.gl.calculate_move()), ["4 h", "3 h", "2 h", "1 h"])
        self.assertListEqual(self.gl.update_logic_move(self.gl.calculate_move()), ["1 d"])
